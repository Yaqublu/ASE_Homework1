...
def vote(id, request):  #extract person and option fields from the JSON request
    ...
    try:    #cast a vote from person in  _ACTIVEPOLLS[id]
        request_data = request.get_json()   # get json...................
        if ('person' in request_data and 'option' in request_data): 
            result = _ACTIVEPOLLS[id].vote(request_data['person'],request_data['option'])
...
def create_doodle(request):
    ...
    _POLLNUMBER += 1
    _ACTIVEPOLLS[str(_POLLNUMBER)] = Poll(_POLLNUMBER, request_data['title'], request_data['options'])
